...
def test_cli_accepts_scale_for_automatic_bandwidth() -> None:
    configured = config_from_args(
        build_parser().parse_args(
            [*BASE_ARGUMENTS, "--bandwidth", "auto", "--bandwidth-scale", "0.5"]
        )
    )
    assert configured.bandwidth == "auto"
    assert configured.bandwidth_scale == 0.5


def test_cli_rejects_scale_with_explicit_bandwidth() -> None:
    arguments = [*BASE_ARGUMENTS, "--bandwidth", "2.5", "--bandwidth-scale", "0.5"]
...
@pytest.mark.parametrize(
    ...
    [
        ...
        [*BASE_ARGUMENTS, "--bandwidth-scale", "0"],
        [*BASE_ARGUMENTS, "--bandwidth-scale", "nan"],
