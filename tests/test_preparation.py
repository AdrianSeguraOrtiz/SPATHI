...
def preparation_inputs(tmp_path: Path) -> dict[str, Path]:
    ...
    annotations.write_text(
        ...
        "c3\tUnit alpha\tstate two\n"
        "c1\tUnit alpha\tstate one\n"
        "c4\trare/type\tonly\n"
        "c2\tUnit alpha\tstate one\n",
        encoding="utf-8",
    )
...
def test_prepare_writes_atomic_andrea_inputs_and_manifest(
    ...
    assert result.prepared_analysis_units == ("Unit alpha",)
    ...
    assert unit_dir.name == "unit-001-unit-alpha"
