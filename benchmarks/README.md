...
`run_scaling_suite.py` runs the validated `smoke`, `progressive`, and `cll-envelope`
synthetic profiles. See the project README for the complete measurement contract.

## Local CLL reference/candidate benchmark

`benchmark_cll_equivalence.py` compares two source snapshots on the same prepared
patient/type units. Raw expression data remain in their local preparation directories.
...
```bash
python benchmarks/benchmark_cll_equivalence.py manifest \
  --prepare-manifest et29=benchmarks/results/cll-real-et29-prepared/prepare_manifest.json \
  --prepare-manifest p34638=benchmarks/results/cll-real-p34638-prepared/prepare_manifest.json \
  --prepare-manifest e20141=benchmarks/results/cll-real-e20141-prepared/prepare_manifest.json \
  --output benchmarks/results/cll-local-datasets.json
```

Inspect the execution matrix without hashing or executing the large inputs:

```bash
python benchmarks/benchmark_cll_equivalence.py run \
  --profile cll-equivalence-smoke \
  --dataset-manifest benchmarks/results/cll-local-datasets.json \
  --reference-source /path/to/reference/SPATHI \
  --candidate-source . \
  --dataset et29-cd8-maturation \
  --dry-run
```
...
python benchmarks/benchmark_cll_equivalence.py run \
  ...
  --dataset et29-cd8-maturation \
  --output-dir benchmarks/results/cll-smoke-single

python benchmarks/benchmark_cll_equivalence.py run \
  --profile cll-equivalence-smoke \
  --dataset-manifest benchmarks/results/cll-local-datasets.json \
  --reference-source . --candidate-source . \
  --dataset p34638-cd4-conventional \
  --output-dir benchmarks/results/cll-smoke-multigroup
```
...
measured pairs. Progressive and production profiles require an explicit dataset:

```bash
python benchmarks/benchmark_cll_equivalence.py run \
  --profile cll-equivalence-progressive \
  --dataset-manifest benchmarks/results/cll-local-datasets.json \
  --reference-source /path/to/reference/SPATHI \
  --candidate-source . \
  --dataset et29-cd8-maturation \
  --dataset e20141-b-cell-maturation
```
...
Progressive and production runs reject identical package hashes by default. Use
...
The production envelope is deliberately opt-in and requires exactly one dataset. It omits `--target-list`, infers every
...
```bash
python benchmarks/benchmark_cll_equivalence.py run \
  --profile cll-equivalence-production-envelope \
  --dataset-manifest benchmarks/results/cll-local-datasets.json \
  --reference-source /path/to/reference/SPATHI \
  --candidate-source . \
  --dataset et29-cd8-maturation \
...
The production profile allows up to 48 hours per child and its disk preflight assumes
...
```bash
python benchmarks/benchmark_cll_equivalence.py resume \
