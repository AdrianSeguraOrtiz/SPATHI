...
BUILTIN_PROFILE_DIR = BENCHMARK_DIR / "profiles" / "v1" / "equivalence"
...
@dataclass(frozen=True, slots=True, kw_only=True)
class LocalDataset:
    """One independently prepared analysis unit."""
...
def _parse_scientific_parameters(raw: Any) -> ScientificParameters:
    ...
    bandwidth_scale = _finite_number(
        value["bandwidth_scale"],
        location=f"{location}.bandwidth_scale",
        strict=True,
    )
    if bandwidth != "auto" and bandwidth_scale != 1.0:
        raise ContractError(f"{location}.bandwidth_scale must be 1 when bandwidth is explicit")
...
def _resolve_profile_path(value: str | Path) -> Path:
    ...
    if not builtin.is_file():
        available = ", ".join(
            sorted(path.stem for path in BUILTIN_PROFILE_DIR.glob("equivalence-*.json"))
        )
...
def build_infer_command(
    ...
    *,
    ...
    command = [
        ...
        "--bandwidth-scale",
        str(scientific_parameters.bandwidth_scale),
    ...
    *,
    ...
    try:
        ...
        expected_requested = {
            ...
            "bandwidth_scale": scientific_parameters.bandwidth_scale,
            "group_size_correction": (
                scientific_parameters.single_group_size_correction
                if single_group
                else scientific_parameters.multi_group_size_correction
...
def build_parser() -> argparse.ArgumentParser:
    ...
    run.add_argument("--profile", default="equivalence-smoke")
