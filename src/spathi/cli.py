...
def build_parser() -> argparse.ArgumentParser:
    ...
    weighting.add_argument(
        ...
        help=(
            "final global positive kernel bandwidth, or median-based automatic selection; "
            "an explicit number is never rescaled"
        ),
    )
    weighting.add_argument(
        "--bandwidth-scale",
        type=_positive_float,
        default=_config_default("bandwidth_scale"),
        help=(
            "positive multiplier applied only to the median or fallback selected by "
            "--bandwidth auto"
        ),
    )
    weighting.add_argument(
