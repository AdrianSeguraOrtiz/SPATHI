...
def _run_workflow_impl(
    ...
    *,
    ...
    automatic_reference = bandwidth.automatic_reference_value
    automatic_scale = bandwidth.automatic_scale
    if bandwidth.fallback_reason:
        if automatic_reference is None or automatic_scale is None:  # pragma: no cover
            raise RuntimeError("automatic bandwidth fallback is missing its scale provenance")
        warning_messages.append(
            f"{bandwidth.fallback_reason}; reference {automatic_reference:g} "
            f"multiplied by bandwidth_scale {automatic_scale:g} produced "
            f"bandwidth {bandwidth.value:g}"
        )
    if automatic_reference is None:
        automatic_detail = ""
    else:
        if automatic_scale is None:  # pragma: no cover
            raise RuntimeError("automatic bandwidth is missing its scale provenance")
        automatic_detail = f"; reference {automatic_reference:.6g} x scale {automatic_scale:.6g}"
    LOGGER.info(
        "Using global %s bandwidth %.6g (%s%s)",
        config.kernel,
        bandwidth.value,
        bandwidth.method,
        automatic_detail,
