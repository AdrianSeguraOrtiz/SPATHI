...
6. writes positive transcription-factor importances as directed candidate edges.
...
Cosine distance requires every cell representation and every group centroid to have a
non-zero norm. SPATHI rejects zero vectors with their identifiers instead of assigning
them an arbitrary similarity. Floating-point residues that are indistinguishable from
exactly zero under the documented dot-product error bound are canonicalized to zero
before bandwidth selection and weighting.
...
reads data. Content-level validation remains part of input loading.
...
| File | Purpose |
|---|---|
| `cell_weights.tsv.gz` | long-form distances, base weights, size factors, and final weights |
| `group_distances.tsv` | pairwise centroid distances |
| `group_affinities.tsv` | target/source group affinities and correction factors |
| `centroids.tsv` | one reusable prototype per group in distance space |
| `weight_diagnostics.tsv` | weight mass, ranges, effective sample size, and group contributions |
...
temporary disk-backed memory map. In `group-distance` mode, those required
cell-to-centroid distances are computed in a bounded streaming pass and discarded
instead of retaining the full matrix; the per-cell `distance` recorded for weighting
remains the applicable centroid-to-centroid distance. Automatic bandwidth still uses
the exact global positive-distance median relevant to the selected mode.
