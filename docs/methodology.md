...
does not reproduce private solver-selection logic. The solver policy is an operational
numerical choice, not a scientific calibration dimension: it remains fixed within an
experiment. Determinism is tested for every supported policy, and full-informative-rank
cases test that solver policies preserve Euclidean and cosine scientific distances
...
copies its selected response columns directly into one final Fortran-contiguous
allocation. SPATHI extracts the reusable TF predictor columns directly into their
final contiguous `float32` matrix, matching
scikit-learn's tree-oriented numeric path without reducing target precision or
retaining a complete intermediate selection. Finiteness scans use a bounded boolean
buffer, and live memory is checked before either persistent inference copy is made.
These conversions do not normalize or otherwise transform the supplied values.
...
The bandwidth \(h\) must be positive. A numeric `bandwidth` is the final value and is
used directly. With `bandwidth=auto`, one reproducible reference bandwidth \(h_0\) is
selected for the entire run:

- for `cell-distance` and `cell-distance-group-anchored`, \(h_0\) is the median of all
  strictly positive cell-to-centroid distances;
- for `group-distance`, \(h_0\) is the median of all strictly positive
  centroid-to-centroid distances.

The effective automatic bandwidth is \(h=s h_0\), where the positive finite
`bandwidth_scale` \(s\) defaults to 1.0. This transferably varies kernel locality while
retaining a dataset-relative automatic reference. Zeros are excluded because
within-group or coincident centroids may legitimately have zero distance. If the
relevant family contains no positive finite distance, SPATHI uses a finite positive
reference fallback, applies the same scale, emits a warning, and records the reference,
scale, effective value, and decision. A numeric `bandwidth` cannot be combined with a
...
cell identifiers can still encode sample or subject information, the report must follow
...
no automatic **edge-score** threshold is applied, and every finite score strictly
greater than zero is retained. The feature importances are relative within each target
model and are not renormalized across targets or groups.
...
numerical-library thread. This fixes
